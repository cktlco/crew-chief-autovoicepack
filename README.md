...
The Docker image itself is ~18GB, specifically including large data dependencies in the image, such as a pre-trained text-to-speech model alongside related machine learning frameworks. This avoids having to download these large files (and wait several minutes) each time the container runs.
